...
/**
 * Per-request authentication context injected into the Elysia handler context
 * as `authdog` by the `attachSession` plugin. `isAuthenticated` is the single
 * source of truth the rest of the app (and `requireAuth`) should branch on.
 */
...
/** Options for the {@link AuthdogServer.attachSession} plugin factory. */
