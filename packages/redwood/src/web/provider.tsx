...
/**
 * Client provider for RedwoodJS web apps. On mount it strips a `?token=…` left
 * by the login redirect from the URL. The raw token is never persisted in the
 * DOM: the server exchanges it for an HttpOnly session cookie, so the client
 * only needs to clean up the URL and reload once so the cookie is picked up.
 *
