...
Authdog SDK for [Hono](https://hono.dev) — session middleware, an authentication
gate, and a logout handler for any Hono runtime (Bun, Node.js, Cloudflare
Workers, Deno). Built on
...
```bash
bun add @authdog/hono hono
```

`hono` is a peer dependency (Hono 4).

## Quick start

```ts
import { Hono } from "hono";
import { createAuthdog } from "@authdog/hono";
...
// Resolve the session for every request and store the `authdog` variable.
app.use(authdog.attachSession());

// Public route — `c.get("authdog")` is always present after `attachSession`.
app.get("/", (c) =>
...
- **`attachSession(options?)`** — reads the session token from the
  ...
  stores the result under the `authdog` context variable:
  ...
  early, so every downstream handler can read `c.get("authdog")`.

- **`requireAuth`** — responds `401 { "error": "Unauthorized" }` when
  `c.get("authdog")?.isAuthenticated` is falsy, otherwise calls `next()`.
...
With `fetchUser: false`, `c.get("authdog").token` is populated but
