...
/**
 * Per-request authentication context attached to the Hono context under the
 * `authdog` variable by the `attachSession` middleware. `isAuthenticated` is the
 * single source of truth the rest of the app (and `requireAuth`) should branch
 * on.
 */
...
declare module "hono" {
  interface ContextVariableMap {
     ...
     * middleware. Available via `c.get("authdog")` once the middleware has run;
     * check `c.get("authdog")?.isAuthenticated` to gate behaviour.
     */
