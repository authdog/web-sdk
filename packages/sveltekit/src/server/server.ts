...
/**
 * Creates a framework-agnostic server helper for SvelteKit `load` functions,
 * form actions and `+server.ts` endpoints. Everything operates on the standard
 * `Request` / `Response` objects SvelteKit already exposes via the request
 * event (`event.request`).
 */
