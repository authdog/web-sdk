 ...
 * result to `ctx.state.authdog`, leaving the decision of what to do with an
...
export const createAttachSession = (
  ...
  return async (ctx, next) => {
    ...
    ctx.state.authdog = await resolveContext(payload, token, fetchUser);
    await next();
  };
};
...
export const requireAuth: Middleware = async (ctx, next) => {
  if (!ctx.state.authdog?.isAuthenticated) {
    ctx.status = 401;
