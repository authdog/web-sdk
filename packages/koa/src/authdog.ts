...
export interface AuthdogServer {
  /**
   * Middleware that resolves the session and attaches `ctx.state.authdog`.
   * Mount it early (typically app-wide) so downstream handlers and
   * `requireAuth` can read the authentication context. Never throws or blocks
   * the request.
   */
  attachSession: (options?: AttachSessionOptions) => Middleware;
  /**
...
/**
 * Creates an Authdog server instance for Koa.
 *
